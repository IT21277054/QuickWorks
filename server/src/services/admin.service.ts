...
//get pending worker
  ...
  async function add(worker: IWorker): Promise<any> {
    try {
...
//change worker status
   const updateWorker=async(id: string)=>{
    ...
    try {
      const worker = await WorkerModel.findById(id)
      console.log(worker)
      if(worker?.status == 'activate'){
        await WorkerModel.findByIdAndUpdate(id,{status:'deactivate'})
      }else{
        await WorkerModel.findByIdAndUpdate(id,{status:'activate'})
...
export default {getNonPendingWorkers,getWorkerById,addWorker,deleteWorker,updateWorker,add};
